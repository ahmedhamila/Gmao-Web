...
import EquipementPopup from "./EquipementPopup";
import AddEquipementPopup from "./AddEquipementPopup";
import { useSelector } from "react-redux";
const URL = `${SERVER_API_CONFIG.PROTOCOL}://${SERVER_API_CONFIG.HOST_NAME}:${SERVER_API_CONFIG.PORT}`;

export default function Equipement({ editMode }) {
  ...
  const handleAddEquipement = (newEquipement) => {
    // Update the bonTravaux state by adding the new bon de travail
    setRows((prevBonTravaux) => [...prevBonTravaux, newEquipement]);
  };

  const handleUpdateRecord = (EquipementId, updatedData) => {
    console.log("updatedData : ", updatedData);
    setRows((prevRows) => {
      const updatedRows = prevRows.map((row) => {
        if (row.id === EquipementId) {
          return {
            ...row,
            ...updatedData,
          };
  ...
  const deleteEquipement = async (EquipementId) => {
    ...
    if (response.ok) {
      notify(false, "Equipement deleted Successfully");
      setRows((prevRows) => prevRows.filter((row) => row.id !== EquipementId));
    } else {
  ...
  const columns = [
    ...
    { field: "date_fabrication", headerName: "Date de fabrication", width: 200 },
    { field: "date_mise_en_marche", headerName: "Date de mise en marche", width: 200 },
      ...
      >
        ...
        <Grid container display="flex" flexDirection="column" gap="25px">
          <Typography variant="body1">
            A Equipement, also known as a Work Order, is a document used to
            ...
            or machinery within a production cycle. The Equipement contains
